...
const validAddHat = (v, a) => a < 0 && (v + a) <= 0;
const shouldAddHatNormal = (m, vs, v, a) => {
	if (!m) return false;
	if (lastNonZeroValue(vs) <= 0) return false;
	return validAddHat(v, a);
};

const validAddHatCrossed = (v, a) => a > 0 && (v + a) >= 0;
const shouldAddHatCrossed = (m, vs, v, a) => {
	if (!m) return false;
	if (lastNonZeroValue(vs) >= 0) return false;
	return validAddHatCrossed(v, a);
};

const isUnplayedRefa = (i) => _.isPlainObject(i) && _.get(i, "middle", 22) === 0;

const checkMiddle = (m, s) => {
	if (!m) throw new Error(s + ":Cannot mark a refa in a soup column! middle=" + m);
};

const checkPosition = (p, s) => {
	if (!validPosition(p)) throw new Error(s + ":Invalid position " + p);
};

class PrefPaperColumn {

	constructor(value, middle = false) {
		if (!validStartValue(value, middle)) throw new Error("PrefPaperColumn::constructor:Value is not valid " + value);
	...
	reset() {
		...
		if (this.middle && validStartValue(this.value, this.middle)) this.values.push(this.value);
		return this;
	}
	...
	addValue(value, invalidated = false) {
		if (!validValue(value, this.middle)) throw new Error("PrefPaperColumn::addValue:Value is not valid " + value);
	...
	processHatAddition(value) {
		if (shouldAddHatNormal(this.middle, this.values, this.value, value)) this.values.push({hat: 1});
		else if (shouldAddHatCrossed(this.middle, this.values, this.value, value)) this.values.push({hat: -1});
		return this;
	}
