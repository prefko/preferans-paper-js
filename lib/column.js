...
const shouldAddHatNormal = (vs, v, a) => {
	let last = lastNonZeroValue(vs);
	return last > 0 && a < 0 && (v + a) <= 0;
};
const shouldAddHatCrossed = (vs, v, a) => {
	let last = lastNonZeroValue(vs);
	return last < 0 && a > 0 && (v + a) >= 0;
};
...
class PrefPaperColumn {
	...
	constructor(value, middle = false) {
		...
		this.initialValue = middle ? value : 0;
		this.reset();
	...
	processHatAddition(value) {
		...
		if (shouldAddHatNormal(this.values, this.value, value)) {
			this.values.push({hat: 1});
		} else if (shouldAddHatCrossed(this.values, this.value, value)) {
			this.values.push({hat: -1});
		}
		return this;
	}
