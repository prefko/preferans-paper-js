...
class PrefPaperPlayer {
	...
	processMyFollowing(data, value, mainPosition, invalidated = false) {
		let {followed, tricks, failed = false} = data;
		if (followed) {
			switch (mainPosition) {
				case "left":
				...
				case "right":
					...
					break;
				default:
					throw new Error("PrefPaperPlayer::processMyFollowing:Invalid position " + mainPosition);
			}
			if (failed) {
