...
class PrefPaperPlayer {
	...
	markPlayedRefa(position, passed) {
		let index = _.findIndex(this.middle, (i) => i.isRefaAvailable(position));
		if (index < 0) {
			throw new Error(_.upperFirst(_.toLower(position)) + " player does not have available refas " + JSON.stringify(this.middle));
		}
