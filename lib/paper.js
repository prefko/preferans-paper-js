...
class PrefPaper {
	...
	getPlayerByUsername(username) {
		let id = _.findKey(this, {'username': username});
		if (id) {
			return this[id];
		}
	...
	processHand(hand) {
		let {value, main, left, right, refa, newRefa, invalidated} = hand;
		...
		if (!!leftPlayer.followed) {
			leftPlayer.addRightValue(new PrefPaperItem(value * leftPlayer.tricks, !!invalidated));
			if (!!leftPlayer.failed) {
				leftPlayer.addMiddleValue(new PrefPaperMiddleItem(-value, !!invalidated));
			}
		}
		if (!!rightPlayer.followed) {
			rightPlayer.addLeftValue(new PrefPaperItem(value * rightPlayer.tricks, !!invalidated));
			if (!!rightPlayer.failed) {
				rightPlayer.addMiddleValue(new PrefPaperMiddleItem(-value, !!invalidated));
			}
		}

		mainPlayer.calculateScore(leftPlayer, rightPlayer);
		leftPlayer.calculateScore(rightPlayer, mainPlayer);
		rightPlayer.calculateScore(mainPlayer, leftPlayer);
	...
	invalidateHand(index) {
		if (!this.hands[index]) {
			throw new Error("PrefPaper::invalidateHand:Hand not found for index " + index);
		}
