...
const validHand = (hand) => {
	...
	let validUsernames = _validUsernames(main, left, right);
	let validObjects = _validObjects(value, main, left, right);
	let validBooleans = _validBooleans(refa, newRefa, invalidated);
	let invalidFollowers = _invalidFollowers(left, right);
	let validTricks = _validTricks(left, right);
	let invalidFails = _invalidFails(main, left, right);
