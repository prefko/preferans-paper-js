...
const validHand = (hand) => {
	...
	if (!_.isInteger(value) || !_.isPlainObject(main) || !_.isPlainObject(left) || !_.isPlainObject(right)) {
		return false;
	} else if (!_.isBoolean(refa) || !_.isBoolean(newRefa) || !_.isBoolean(invalidated)) {
		return false;
	} else if (!main.username || !left.username || !right.username) {
		return false;
	} else if ((left.followed && !_.isInteger(left.tricks)) || (right.followed && !_.isInteger(right.tricks))) {
		return false;
	} else if (left.tricks + right.tricks > 5) {
		return false;
	} else if (main.failed && (left.failed || right.failed)) {
		return false;
	}
	return true;
};
