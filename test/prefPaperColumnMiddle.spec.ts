...
describe("PrefPaperColumnMiddle tests", () => {
	...
	describe("PrefPaperColumnMiddle addValue tests", () => {
		...
		const column1 = new PrefPaperColumnMiddle(60).addValue(10).addValue(-10);
		it("PrefPaperColumnMiddle addValue should work properly", () => {
			expect(column1.value).to.equal(60);
			expect(column1.json).to.deep.equal([60, 70, 60]);
		});

		const column2 = new PrefPaperColumnMiddle(60).addValue(10).addValue(-10, true);
		it("PrefPaperColumnMiddle addValue repealed should work properly", () => {
			expect(column2.value).to.equal(70);
			expect(column2.json).to.deep.equal([60, 70, {value: 60, repealed: true}]);
		});

		const column3 = new PrefPaperColumnMiddle(30).addValue(-30);
		...
		const column4 = new PrefPaperColumnMiddle(30).addValue(-40);
		it("PrefPaperColumnMiddle addValue below 0 should work properly", () => {
			expect(column4.value).to.equal(-10);
			expect(column4.json).to.deep.equal([30, {hat: 1}, -10]);
		});

		const column5 = new PrefPaperColumnMiddle(30).addValue(-40).addValue(10);
		it("PrefPaperColumnMiddle addValue below 0 back to 0 should work properly", () => {
			expect(column5.value).to.equal(0);
			expect(column5.json).to.deep.equal([30, {hat: 1}, -10, {hat: -1}]);
		});

		const column6 = new PrefPaperColumnMiddle(30).addValue(-40).addValue(20);
		it("PrefPaperColumnMiddle addValue below 0 back to 0 should work properly", () => {
			expect(column6.value).to.equal(10);
	...
	describe("PrefPaperColumnMiddle markPlayedRefa tests", () => {
		...
		const column2 = new PrefPaperColumnMiddle(60).addValue(10)
			.addRefa().addValue(10)
